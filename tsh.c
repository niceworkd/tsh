...
void eval(char *cmdline) 
{
    ...
    if((command = builtin_cmd(argv)) != -1){
        ...
        bg ? addjob(jobs,p,BG,cmdline) : addjob(jobs,p,FG,cmdline);
        if(bg){
            printf("[%d] (%d) %s",pid2jid(p),p,cmdline);
        }
        sigprocmask(SIG_SETMASK,&prev_one,NULL);
        if(!bg){
            waitfg(p);
...
int builtin_cmd(char **argv) 
{
    char* bti_cmd[] = {"jobs","bg","fg","quit"};
    for(int i=0;i<4;i++){
        if(strcmp(*argv, bti_cmd[i]) == 0)
            return i;
    }
...
{
    if(argv[1] == NULL){
        printf("input valid num\n");
        return;
    }
    char* s_arg = argv[1];
    int num= atoi(s_arg+1);
    if(num == 0){
        printf("input valid num\n");
        return;
    }
...
void sigtstp_handler(int sig) 
{
    ...
    if(kill(-pid,sig) != 0){
        unix_error("send tstp sig error\n");
    }
    return;
}
